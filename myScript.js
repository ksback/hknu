            ...
            yearDataMap[year].forEach(item => {
                for (let i = 0; i < headers.length; i++) { // 0부터 시작하여 모든 헤더를 포함
                    ...
                    //팝업 위치 ~
                            ...
                            // imgPath 설정: dataBox ID와 year 연결
                            const baseImgPath = `/img/${year}/${dataBox.id}`; // 기본 경로 설정

                            // A부터 G까지의 이미지를 추가
                            let imgLoaded = false;
                            imgFormats.forEach(format => {
                                const imgPath = `${baseImgPath}.${format}`; // 각 형식에 대한 전체 경로
                                ...
                                img.onload = () => {
                                    ...
                                    img.style.width = `${boxWidth}px`; // 고정 가로값 설정
                                    img.style.height = `${boxWidth * imgSize}px`; // 비율에 따라 세로값 계산
