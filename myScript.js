...
for (let year = 1939; year <= 2024; year++) {
    ...
    const extensions = ['png', 'jpg', 'jpeg']; // 확장자 배열
        ...
        for (let i = 1; i < arrayData.length; i++) {
            if (arrayData[i][0] === year) { // 0열에서 연도 확인
                ...
                if (columnIndex !== -1) {
                    ...
                    // 이미지 경로 생성
                    const imagePaths = [
                        `img/${year}/${dataBoxId}.jpg`,
                        `img/${year}/${dataBoxId}.jpeg`,
                        `img/${year}/${dataBoxId}.png`
                    ];
                    // 이미지가 캐시에 있는지 확인
    ...
    yearBoxes.forEach((yearBox) => {
        ...
        categories.forEach((category) => {
            for (let i = 1; i <= 7; i++) {
                ...
                const cachedImage = getImage(`img/${year}/${dataBoxId}.jpg`) || 
                                    getImage(`img/${year}/${dataBoxId}.jpeg`) || 
                                    getImage(`img/${year}/${dataBoxId}.png`);
            ...
            if (firstBox) {
                ...
                for (let i = 1; i < arrayData.length; i++) { // 1행부터 시작 (헤더 제외)
                    if (arrayData[i][0] === yearBox.id) { // 연도 확인
                        ...
                        // 캐시에서 이미지 확인 후 로드
                        const cachedImage = getImage(`img/${yearBox.id}/${dataBoxId}.jpg`) || 
                                            getImage(`img/${yearBox.id}/${dataBoxId}.jpeg`) || 
                                            getImage(`img/${yearBox.id}/${dataBoxId}.png`);
